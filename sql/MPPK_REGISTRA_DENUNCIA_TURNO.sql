CREATE OR REPLACE PACKAGE SISMPA.MPPK_REGISTRA_DENUNCIA_TURNO IS
    ...
    *****************************************************************************************/
    ...
    /****************************************************************************
    * Nombre        : SISMPA.MPPK_REGISTRA_DENUNCIA_TURNO.MPSP_REGISTRAR_DELITOS
    * Autor         : Pedro Yarleque Linares
    * Versión       : 1.0
    * Descripción   : Registrar los delitos de la denuncia, y la asociación de los
    *                 detenidos con los delitos
    * Fecha creación: 09.01.2024
    *****************************************************************************/
...
IF V_N_CONTADOR = 0 THEN
		        PO_V_ERR_COD := '42202043';
		        SISCFE.CFPK_MENSAJE_VALIDACION.CFSP_OBTENER_MENSAJE_VALIDACION(PO_V_ERR_COD, PO_V_ERR_MSG, V_V_COD_ERR, V_V_MSG_ERR);
		       	PO_V_ERR_MSG := PO_V_ERR_MSG || '-' || V_V_ID_DENUNCIA || '--' || V_V_ID_CASO ;
				RAISE EX_CONTROLADO;
END IF;
...
SELECT SISEFE.EFFN_PLAZO_FE_FIN(SYSDATE , V_C_DIA_HABIL, V_N_NU_CANTIDAD_MAX)
INTO V_D_FE_FINAL
FROM DUAL;
...
END IF;
	    ...
	    SISMPA.MPPK_REGISTRA_DENUNCIA_TURNO.MPSP_REGISTRAR_DELITOS(
	        V_V_ID_DENUNCIA,
		    V_V_ID_CASO,
	    	PI_CR_DELITOS,
	    	PI_CR_DETENIDOS,
...
PO_V_FE_CREACION :=  TO_CHAR(V_D_FECHA_EMISION, 'DD/MM/YYYY');
...
WHEN OTHERS THEN
	        ...
	        --PO_V_ERR_MSG := 'ERROR AL INTENTAR ' || V_V_ACCION || ' LOS DATOS DE LA DENUNCIA: ' || to_char(SQLCODE) || ' - ' ||SQLERRM || PO_V_ERR_COD || ' / ' || PO_V_ERR_MSG;
ROLLBACK TO REGISTRO_DENUNCIA_TURNO;
...
END MPSP_REGISTRAR_PARTES_INVOLUCRADAS;
    ...
    *****************************************************************************/
    ...
    PROCEDURE MPSP_REGISTRAR_DELITOS(
       ...
       V_N_CONTADOR             NUMBER;
       V_N_CONTADOR_CONDICION   NUMBER;
       V_N_CONTADOR_DETENIDO    NUMBER;
       ...
       V_V_ID_DELITO_SUJETO     SISCFE.CFTV_DELITO_DETENIDO.ID_V_DELITO_SUJETO%TYPE;
...
WHERE ID_V_DENUNCIA = PI_V_ID_DENUNCIA
  ...
  AND ID_N_TIPO_PARTE_SUJETO IN (2,4)
  AND ID_V_PERSONA IS NOT NULL; -- Denunciado, Imputado
...
END LOOP;
	    ...
	    -- VINCULAR TODOS LOS DELITOS AL SUJETO

FOR SUJETO IN CR_SUJETOS_CASO LOOP

		    V_N_ID_TIPO_COND_SUJETO := 0;

		    -- VALIDAR CONDICION DE SUJETO

SELECT COUNT(1)
INTO V_N_CONTADOR_CONDICION
FROM SISMPA.MPTV_SUJETO_CONDICION
WHERE ID_V_SUJETO_CASO = SUJETO.ID_V_SUJETO_CASO
  AND ES_C_SUJETO_CONDICION = '1';

IF V_N_CONTADOR_CONDICION > 0 THEN
...
-- VALIDAR SI SUJETO ESTA EN EL ARRAY TYPE DE DETENIDOS

V_N_CONTADOR_DETENIDO := 0;

		    IF PI_CR_DETENIDOS IS NOT NULL THEN
		    	FOR I IN 1..PI_CR_DETENIDOS.COUNT LOOP
			    	IF (PI_CR_DETENIDOS(I).ID_N_TIPO_DOC_IDENT = V_N_ID_TIPO_DOC_IDENT
			    	   ...
			    	   AND PI_CR_DETENIDOS(I).ID_N_TIPO_PARTE_SUJETO = V_N_ID_TIPO_COND_SUJETO)
			    	THEN
			    		V_N_CONTADOR_DETENIDO := 1;
			    		EXIT;
END IF;
END LOOP;
END IF;

		    IF V_N_CONTADOR_CONDICION > 0 AND V_N_ID_TIPO_COND_SUJETO = 3 AND V_N_CONTADOR_DETENIDO > 0 THEN -- DETENIDO
...
WHERE ID_V_SUJETO_CASO = SUJETO.ID_V_SUJETO_CASO
  AND ID_N_DELITO = PI_CR_DETENIDOS(I).CR_DELITOS_ASOCIADOS(J).ID_N_DELITO
  AND ID_N_SUBGENERICO = PI_CR_DETENIDOS(I).CR_DELITOS_ASOCIADOS(J).ID_N_SUBGENERICO
  AND ID_N_ESPECIFICO = PI_CR_DETENIDOS(I).CR_DELITOS_ASOCIADOS(J).ID_N_ESPECIFICO
...
IF V_N_CONTADOR = 0 THEN
						         ...
						         VALUES(
						             ...
						             SYSDATE,
						             '1'
						         ) RETURNING ID_V_DELITO_SUJETO INTO V_V_ID_DELITO_SUJETO;

					        ELSIF V_N_CONTADOR > 0 THEN

SELECT ID_V_DELITO_SUJETO
INTO V_V_ID_DELITO_SUJETO
FROM SISEFE.EFTV_DELITO_SUJETO
WHERE ID_V_SUJETO_CASO = SUJETO.ID_V_SUJETO_CASO
  AND ID_N_DELITO = PI_CR_DELITOS(I).ID_N_DELITO
  AND ID_N_SUBGENERICO = PI_CR_DELITOS(I).ID_N_SUBGENERICO
  AND ID_N_ESPECIFICO = PI_CR_DELITOS(I).ID_N_ESPECIFICO
  AND ES_C_DELITO_SUJETO = '1';

END IF;

					        -- REGISTRAR DELITO DETENIDO
...
END IF;
				    ...
				    -- REGISTRAR DELITOS PARA DETENIDOS CUANDO NO HAN SIDO ASOCIADOS

				    IF ( V_N_CONTADOR_CONDICION > 0 AND V_N_ID_TIPO_COND_SUJETO = 3 AND V_V_ID_DELITO_SUJETO <> 'NULL' ) THEN
...
--                                 PO_V_ERR_MSG);
        WHEN
            OTHERS THEN
            PO_V_ERR_COD := '1';
            --PO_V_ERR_MSG := 'REGISTRAR_DELITOS - ERROR : ' || SQLERRM;
            PO_V_ERR_MSG := 'REGISTRAR_DELITOS - ERROR : ' || SQLCODE || ' - ' || SQLERRM || '-' || 'Objeto: ' || dbms_utility.format_error_backtrace() || '--' || dbms_utility.format_error_stack();
